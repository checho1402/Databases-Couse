...
txt_almacen = "tabla_coseno_"+word1+".txt"
f=open(txt_almacen,'w',encoding ="utf-8")
...
try:
    with conexion.cursor() as cursor:
        ...
        #cursor.execute("select raiz from raices where raiz <> '---' and raiz <> ?",word1)
...
for i in range (len(word2)):
    ...
    try:
        with conexion.cursor() as cursor:
            ...
            #cursor.execute("select lema_1,lema_2,sum(W) from (	select lema_1, lema_2, W from hallar_w(?) union	select ?,lema_2,0 from hallar_w(?) ) as R group by lema_1,lema_2 order by lema_2",word1,word1, word2[i][0])
            cursor.execute("select R.lema_1, R.lema_2, R.w from ( select E.lema_1,E.lema_2,E.W from hallar_w(?) E join hallar_w(?) D on E.lema_2=D.lema_2) as R order by lema_2",word1,word2[i][0])
            # Con fetchall traemos todas las filas
            palabra_1 = cursor.fetchall()

            #cursor.execute("select lema_1,lema_2,sum(W) from (	select lema_1, lema_2, W from hallar_w(?) union	select ?,lema_2,0 from hallar_w(?) ) as R group by lema_1,lema_2 order by lema_2",word2[i][0],word2[i][0], word1)
            cursor.execute("select R.lema_1, R.lema_2, R.w from ( select E.lema_1,E.lema_2,E.W from hallar_w(?) E join hallar_w(?) D on E.lema_2=D.lema_2) as R order by lema_2",word2[i][0], word1)
            # Con fetchall traemos todas las filas
    ...
    calculo=calcular_coseno(palabra_1,palabra_2)
    #print(calculo)
    f.write(word2[i][0]+": ")
    f.write(str(calculo))
    f.write("\n")
